...
- `rfc-0030-repair-worker-report-contract`: ready repair-worker report contract spec for issue #25, keeping repair agents on the canonical worker report schema and adding regression coverage for malformed repair reports.
- `rfc-0031-issue-evidence-capability-preflight`: ready issue-evidence capability preflight spec for issue #26, requiring GitHub issue evidence workstreams to have explicit network/read capability or a local-only evidence strategy.
- `rfc-0032-validator-artifact-evidence-gate`: ready validator artifact evidence gate spec for issue #27, preventing validators from passing cleanly without the required artifact evidence.
- `rfc-0033-host-heartbeat-snapshot-producer`: ready host-heartbeat snapshot producer spec for issue #28, adding a Codex-facing writer contract for dashboard heartbeat freshness state.
