...
- `rfc-0025-safe-parallel-dispatch-contract`: ready-review safe parallel dispatch contract for issue #22, with explicit parallelism analysis, ready-set coordinator dispatch guidance, serial rationale requirements, and warning-only plan diagnostics implemented.
- `rfc-0026-dashboard-status-plan-consistency`: ready-review dashboard/status consistency fix for issue #24, with shared workstream normalization implemented across `de status --json`, `/api/status`, and `/api/plan`.
- `rfc-0027-dashboard-observer-lifecycle`: ready-review dashboard observer lifecycle contract for issue #23, covering cancelled, terminal, and superseded run dashboards while keeping heartbeat as the operator wakeup mechanism.
- `rfc-0028-coordinator-approval-decision-record`: ready-review coordinator approval decision-record contract for issue #21, with diagnostics for approval blockers that lack durable pending decisions.
- `rfc-0029-validation-worker-stall-terminalization`: ready-review validation-worker stall terminalization spec for issue #20, with stale validation diagnostics, dashboard validator evidence, and terminal report evidence guidance implemented.
- `rfc-0030-repair-worker-report-contract`: ready-review repair-worker report contract spec for issue #25, keeping repair agents on the canonical worker report schema and adding regression coverage for malformed repair reports.
- `rfc-0031-issue-evidence-capability-preflight`: ready-review issue-evidence capability preflight spec for issue #26, requiring GitHub issue evidence workstreams to have explicit network/read capability or a local-only evidence strategy.
- `rfc-0032-validator-artifact-evidence-gate`: ready-review validator artifact evidence gate spec for issue #27, preventing validators from passing cleanly without the required artifact evidence.
- `rfc-0033-host-heartbeat-snapshot-producer`: ready-review host-heartbeat snapshot producer spec for issue #28, adding a Codex-facing writer contract for dashboard heartbeat freshness state.
