...
- State that Codex uses a `codex exec` command shape and Claude uses a `claude -p` command shape, with exact prompt/input/context arguments finalized by implementation.
...
rg -n "provider codex|provider claude|codex exec|claude -p|coordinator-only|agent.spawned|protocol.violation|heartbeats|agents/" SKILL.md README.md references specs/rfc-0005-provider-cli-coordinator-protocol
git diff --check
```
