...
Production semantics: this surface is role-specific. It renders registered
agents whose `.dispatch/runs/<run-id>/agents/<agent-id>.json` role is
`validator` plus role-specific evidence under
`.dispatch/runs/<run-id>/validation/<agent-id>.json`. A run that validates
through worker reports, coordinator report summaries, or main-session review
without spawning validator agents must show an explicit empty state rather
than synthesizing validator rows from worker validation commands.
...
| Validators panel | `screens.jsx`, `data.js` `VALIDATORS` | `.validator-row`, `renderValidators` | `/api/status` agents with `role: "validator"` and validation reports under `.dispatch/runs/<run-id>/validation/` | live/empty-state | coordinator-owned screenshot pending | coordinator-owned screenshot pending | static asset serving test, history validation-count test | pass-empty | Empty is expected for runs that validate through worker reports or main-session review without validator agents; command duration/count fields are present only when validator reports expose them. |
...
| `VALIDATORS.name`, command, agent, status, duration, count, note | validator-role agents via `/api/status` plus role-specific validation evidence under `.dispatch/runs/<run-id>/validation/` and `/api/report/<agent-id>` | live/empty-state | Duration/count/note depend on validator report schema completeness. If no validator agents exist, the correct output is an empty validator panel. |
